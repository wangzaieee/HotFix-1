            ...
            if (f.getName().endsWith('.class') && shouldProcessClass(f.absolutePath, minify)) {
                ...
                if (writer != null) {
                    ...
                    if (!md5.equals(value)) {
                        ...
                        System.out.println("====$f.absolutePath,$dest====")
...
//            println("=====$f.name=====")
            ...
            }else if(f.name.endsWith('.jar') && shouldProcessJar(f.absolutePath)){//add by Lwan 2016/12/9
                processJar(f, writer, inject, generatePatch, md5Map, patchDir, minify)
            }
        }
    }
}
